...
This tutorial will discuss 2 separate ways to conduct cell type VAFs: 1)
The first is a variant approach style where cells will be labeled in
...
We will first start with the same file we use in the basic starting
...
Now we will convert sce to seurat and follow the standard cell type
identification
...
After labeling cell types we can compare the overlap to our clone codes.
...
We can further look at a heatmap to highlight the distribution of clones
along each cell type. The Lymphoid cells are primarily in WT, while
Monocytic cells occupy the 1_0_1 which are TET2 NRAS mutant. The other
...
Within our variant_ID function, we can take advantage of the
...
Let’s build an enrichment plot to compare 2 separate cell populations,
monocytic blasts and lymphoid, that should be distinct in mutational
profiling. As some variants may not have been found in either
