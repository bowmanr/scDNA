...
We assume you have already familiarized yourself with our single sample
...
run variant ID like normal, with no additional inputs, BUT we choose
...
Preform demultiplex to get cluster assignment for each cell. We use an
iterative learning strategy to reduce our variants by cells matrix to
...
Now perform we perform variant_ID but pump in the cell-cluster
...
Running a cohort analysis on a demultiplexed sample follows our base
