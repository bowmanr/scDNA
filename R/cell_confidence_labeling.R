...
#'
#' @param sce the input can be either a single cell experiment object, or the h5 file name to pull information from.
#'
#' @return if the input is an sce object, then sce object is returned with updated labeling in the colData. If the file name is only provided, then a dataframe is returned.
#' @export
#' @importFrom magrittr %>%
#'
#' @examples
...
cell_confidence_labeling<-function(sce){
  ...
  if(class(sce)=="SingleCellExperiment"){
    ...
    sample_file = sce@metadata$file
  }else if(class(sce)=="character"){
    message("File name detected")
    sample_file = sce
  }else{
    stop("The input is not a singleCellExperiment nor an h5 file. Please change the input type")
  }

  all_protein_droplets <- rhdf5::h5read(file = sample_file,
                                        name = "/all_barcodes/protein_read_counts/layers/read_counts")
  all_dna_droplets <- rhdf5::h5read(file = sample_file,
                                    name = "/all_barcodes/dna_read_counts/layers/read_counts")
  colnames(all_dna_droplets) <- rhdf5::h5read(file = sample_file,
                                              name = "/all_barcodes/dna_read_counts/ra/barcode")
  colnames(all_protein_droplets) <- rhdf5::h5read(file = sample_file,
                                                  name = "/all_barcodes/protein_read_counts/ra/barcode")
  rownames(all_protein_droplets) <-rhdf5::h5read(file=sample_file,
  ...
  outlier_score_neglog<-(-1)*log(outlier_score)%*%(abs(skew_meas)/Matrix::norm(skew_meas,type="2"))

  outlier_cutoff_val<-stats::quantile(outlier_score_neglog)[4]+1.5*stats::IQR(outlier_score_neglog)
  ...
  if(class(sce)=="SingleCellExperiment"){
    ...
    SummarizedExperiment::colData(sce)$dna_score <-reduced_df$score
    SummarizedExperiment::colData(sce)$protein_entropy<-reduced_df$ent_score
    SummarizedExperiment::colData(sce)$outlier_score <-reduced_df$outlier_score
    SummarizedExperiment::colData(sce)$cell_label_confidence <- ifelse(reduced_df$outlier_score<=outlier_cutoff_val,"High_confidence","Low_confidence")
    SummarizedExperiment::colData(sce)$cell_label_confidence <- ifelse(reduced_df$ecdf_p_value<= 0.025, SummarizedExperiment::colData(sce)$cell_label_confidence,"Poor_DNA")
    return(sce)
  }
  else{
    reduced_df<- reduced_df%>%
      dplyr::mutate(cell_label_confidence=ifelse(outlier_score<=outlier_cutoff_val,"High_confidence","Low_confidence"),
                    cell_label_confidence= ifelse(ecdf_p_value<= 0.025, cell_label_confidence,"Poor_DNA"))
    return(reduced_df)
  }
